$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            data.forEach(course => {
                $('#course').append(`<option value="${course.course_code}">${course.course_name}</option>`);
            });
        },
        error: function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 'success' && Array.isArray(response.data)) {
                    const facultyOptions = response.data.map(member =>
                        `<option value="${member.faculty_id}">${member.faculty_name}</option>`
                    ).join('');
                    $('select[name$="[faculty_id]"]').each(function () {
                        $(this).html(`<option value="">Select Faculty</option>` + facultyOptions);
                    });
                } else {
    ...
    $('#saveTimetable').click(function () {
        ...
        $('#subjectsForm').find('.timetable-entry').each(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data && data.start_time) {
                    ...
                    timetableRow.find('select[name*="[faculty_id]"]').val(data.faculty_id);
    ...
    // Handle "Generate Timetable" button click event
...
document.getElementById('cancelSubjects').addEventListener('click', function() {
    document.getElementById('subjectsForm').style.display = 'none';
    document.getElementById('timetableForm').style.display = 'block';
});

document.getElementById('cancelTimetable').addEventListener('click', function() {
    document.getElementById('timetableDisplay').style.display = 'none';
    document.getElementById('timetableForm').style.display = 'block';
});