...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $amount = format_NP_Number($_POST['amount']) ?? null;
...
<head>
  ...
  <style>
    ...
    .left { width:24%; }
    .right { width:55%; }
    img.logo { width: 156px; margin-bottom: 81px; margin-top: -54px; }
    h2 { font-size:22px; }
    ...
    .fee-item:hover { background:#f9f9f9; }
    .fee-item.selected { background:#e0f7fa; }
    ...
    .summary { margin-top: 70px; margin-left: 116px; }
    .leftcontainer { margin-left: 142px; }
    .form-control { padding:10px; width:200px; }
    .pay-btn {
      margin-top:20px;
      background:black;
      color:white;
    ...
    .footer { text-align:center; margin-top:120px; font-size:12px; color:#666; }
    .fee-checkbox {
      margin-right: 10px;
    }
    .fee-left {
...
<footer>
  <div class="footer mt-auto">
    <p>Instructions | Terms of Use | Privacy</p>
    <p style="text-align:left; margin-left: 20px;">© 2025 All Rights Reserved.</p>
  </div>
</footer> -->
...
document.getElementById('payBtn').addEventListener('click', function(event) {
    event.preventDefault();  // prevent form submission/navigation if any

    fetch('epay-handler.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ amount: amount_Send })
    })
    .then(response => {
 ...
 <input type="text" id="success_url" name="success_url" value="http://localhost/CMS/Dashboard/Student_Dashboard/Student_php/pay/verify.php" required>
 <input type="text" id="failure_url" name="failure_url" value="http://localhost/CMS/Dashboard/Student_Dashboard/Student_php/pay/verify.php" required>
