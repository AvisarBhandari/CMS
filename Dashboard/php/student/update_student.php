...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE students_info 
                            SET student_roll = ?, student_name = ?, gender = ?, email = ?, department_name = ?, course_code = ?, phone_no = ?, semester = ?, dob = ?, admission_date = ?, parent_name = ?, address = ?
                            WHERE student_id = ?");
    ...
    $stmt->bind_param('sssssssissssi', $student_roll, $student_name, $gender, $email, $department, $course, $phone_no, $semester, $dob, $admission_date, $parent_name, $address, $student_id);
